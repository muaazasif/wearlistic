<?php
class snsavaz_MegaMenu {
    function __construct() {
        add_filter( 'wp_setup_nav_menu_item', array( $this, 'snsavaz_megamenuset' ) );
		add_action( 'wp_update_nav_menu_item', array( $this, 'snsavaz_megamenusave'), 10, 3 );
		add_filter( 'wp_edit_nav_menu_walker', array( $this, 'snsavaz_megamenuedit'), 10, 2 );
		add_action( 'admin_print_footer_scripts', array( $this, 'snsavaz_megamenujs' ), 99 );
		add_action( 'admin_print_styles', array( $this , 'snsavaz_loadadmincss'));
		add_action( 'admin_print_scripts', array( $this , 'snsavaz_loadadminjs'));
	
    }
    // Set value
	function snsavaz_megamenuset ( $item ) {
		// For 1st level
	...
	// Save option to db	
    function snsavaz_megamenusave( $menu_id, $menu_item_db_id, $args ) {
		// Enable
	...
	// Edit form
    function snsavaz_megamenuedit($walker, $menu_id) {
	    return 'snsavaz_Megamenu_Admin'; 
	}
	// Load css file
	function snsavaz_loadadmincss(){
		wp_enqueue_style('thickbox');
		wp_enqueue_style( 'icon-picker', SNSAVAZ_THEME_URI.'/assets/css/admin_megamenu.css', false, '1.0', 'all' );
		wp_enqueue_style('fonts-awesome', SNSAVAZ_THEME_URI . '/assets/fonts/awesome/css/font-awesome.min.css');
		
	}
	// Load js file
	function snsavaz_loadadminjs(){
		wp_enqueue_script('thickbox');
		
	}
	// Load js inline
    function snsavaz_megamenujs() {
	    ...
	    ?>
		<div id="sns_iconmegapicker" class="hidden-el">
...
// Init snsavaz_MegaMenu
$sns_mega = new snsavaz_MegaMenu();
require_once SNSAVAZ_THEME_DIR . '/framework/mega-menu/admin.php';
require_once SNSAVAZ_THEME_DIR . '/framework/mega-menu/frontend.php';
