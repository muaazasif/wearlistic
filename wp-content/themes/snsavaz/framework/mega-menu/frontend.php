<?php
class snsavaz_Megamenu_Front extends Walker_Nav_Menu { 
    var $columns = 0;
    var $enablemega = 0;
	var $stylemega = '';
	var $sidebaremega = '';
    ...
    function start_lvl(&$output, $depth = 0, $args = array()) {
		...
		if($depth === 0 && $this->enablemega && $this->stylemega !=''){
			$output .= "\n$indent<div class=\"sub-content dropdownmenu\"><ul class=\"". $this->stylemega ." {replace_class}\">\n";
		}else{
    	...
    	switch ($type){
    		case 'post':
    			if(has_post_thumbnail($item_id))
    				$html = get_the_post_thumbnail($item_id, 'snsavaz_megamenu_thumb');
    			break;
    		case 'product':
    				if(has_post_thumbnail($item_id))
    					$html = get_the_post_thumbnail($item_id, 'snsavaz_megamenu_thumb');
    		...
    		case 'product_cat':
    			...
    			$cat_thumbnail = wp_get_attachment_image_src($thumbnail_id, 'snsavaz_megamenu_thumb');
