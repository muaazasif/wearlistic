...
if( class_exists('WooCommerce') ){
	$loop = snsavaz_woo_query($type, $number_limit);
	...
	if($template == '1'){ // Template carousel 
		...
		ob_start();
		wc_get_template( 'vc/carousel.php', array('loop' => $loop, 'number_display' => $number_display,'row' => $row, 'id' => 'sns_products'.$uq, 'show_countdown' => $show_countdown) );
		$output .= ob_get_clean();
		$output .= '</div>';
		endif;
