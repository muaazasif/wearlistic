...
<div class="dokan-map-wrap">
    <div class="dokan-map-search-bar">
        <input id="dokan-map-add" type="text" class="form-control dokan-map-search" value="<?php echo esc_attr( $map_address ); ?>" name="find_address" placeholder="<?php esc_attr_e( 'Address', 'cartzilla' ); ?>" size="30" />
        <a href="#" class="dokan-map-find-btn d-none" id="dokan-location-find-btn" type="button"><?php esc_html_e( 'Find Address', 'cartzilla' ); ?></a>
    </div>
    ...
    jQuery(document).ready(function ($) {
        ...
        dokanMapbox.on( 'load', function () {
            dokanGeocoder = new MapboxGeocoder( {
                ...
                placeholder: '<?php esc_html_e( 'Search Address', 'cartzilla' ); ?>',
                marker: false,
                reverseGeocode: true,
            });
